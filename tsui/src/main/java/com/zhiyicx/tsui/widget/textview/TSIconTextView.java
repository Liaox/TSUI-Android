 ...
 * @Description 带图标的文本控件
 */
public class TSIconTextView extends View {
    ...
    public interface OnIconTextClickListener {
        void onIconTextClick(TSIconTextView view);
    }

    private OnIconTextClickListener mOnIconTextClickListener;

    public TSIconTextView(Context context) {
        this(context, null);
    }

    public TSIconTextView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public TSIconTextView(Context context, AttributeSet attrs, int defStyleAttr) {
        super(context, attrs, defStyleAttr);
        TypedArray ta = context.obtainStyledAttributes(attrs, R.styleable.TSIconTextView);

        mTextSize = ta.getDimensionPixelSize(R.styleable.TSIconTextView_tsIconTextSize,
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 14, context
                        .getResources().getDisplayMetrics()));
        mTextColor = ta.getColor(R.styleable.TSIconTextView_tsIconTextColor, 0xff0099cc);
        mGap = ta.getDimensionPixelSize(R.styleable.TSIconTextView_tsIconGap,
                (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 2, context
                        .getResources().getDisplayMetrics()));
        mIconWidth = ta.getDimensionPixelSize(R.styleable.TSIconTextView_tsIconWidth, 0);
        mIconHeight = ta.getDimensionPixelSize(R.styleable.TSIconTextView_tsIconHeight, 0);

        mDrawable = ta.getDrawable(R.styleable.TSIconTextView_tsIcon);
        mDirection = Direction.values()[ta.getInt(R.styleable.TSIconTextView_tsIconDirection, 0)];
        mText = ta.getString(R.styleable.TSIconTextView_tsIconText);
