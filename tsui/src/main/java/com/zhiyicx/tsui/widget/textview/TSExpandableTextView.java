...
/**
 * @author Catherine
 * @describe 可以展开和收起的TextView
 * @date 2017/8/15
 * @contact email:648129313@qq.com
 */

public class TSExpandableTextView extends AppCompatTextView {
    ...
    //  used to judge if the listener of corresponding to the onclick event of TSExpandableTextView
    //  is specifically for inner toggle
    private ExpandableClickListener mExpandableClickListener;
    private OnExpandListener mOnExpandListener;

    public TSExpandableTextView(Context context) {
        super(context);
        init();
    }

    public TSExpandableTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAttr(context, attrs);
        init();
    }

    public TSExpandableTextView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttr(Context context, AttributeSet attrs) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.TSExpandableTextView);
        if (a == null) {
            return;
        }
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (attr == R.styleable.TSExpandableTextView_tsMaxLinesOnShrink) {
                mMaxLinesOnShrink = a.getInteger(attr, MAX_LINES_ON_SHRINK);
            } else if (attr == R.styleable.TSExpandableTextView_tsEllipsisHint) {
                mEllipsisHint = a.getString(attr);
            } else if (attr == R.styleable.TSExpandableTextView_tsToExpandHint) {
                mToExpandHint = a.getString(attr);
            } else if (attr == R.styleable.TSExpandableTextView_tsToShrinkHint) {
                mToShrinkHint = a.getString(attr);
            } else if (attr == R.styleable.TSExpandableTextView_tsEnableToggle) {
                mToggleEnable = a.getBoolean(attr, TOGGLE_ENABLE);
            } else if (attr == R.styleable.TSExpandableTextView_tsToExpandHintShow) {
                mShowToExpandHint = a.getBoolean(attr, SHOW_TO_EXPAND_HINT);
            } else if (attr == R.styleable.TSExpandableTextView_tsToShrinkHintShow) {
                mShowToShrinkHint = a.getBoolean(attr, SHOW_TO_SHRINK_HINT);
            } else if (attr == R.styleable.TSExpandableTextView_tsToExpandHintColor) {
                mToExpandHintColor = a.getInteger(attr, TO_EXPAND_HINT_COLOR);
            } else if (attr == R.styleable.TSExpandableTextView_tsToShrinkHintColor) {
                mToShrinkHintColor = a.getInteger(attr, TO_SHRINK_HINT_COLOR);
            } else if (attr == R.styleable.TSExpandableTextView_tsToExpandHintColorBgPressed) {
                mToExpandHintColorBgPressed = a.getInteger(attr, TO_EXPAND_HINT_COLOR_BG_PRESSED);
            } else if (attr == R.styleable.TSExpandableTextView_tsToShrinkHintColorBgPressed) {
                mToShrinkHintColorBgPressed = a.getInteger(attr, TO_SHRINK_HINT_COLOR_BG_PRESSED);
            } else if (attr == R.styleable.TSExpandableTextView_tsInitState) {
                mCurrState = a.getInteger(attr, STATE_SHRINK);
            } else if (attr == R.styleable.TSExpandableTextView_tsGapToExpandHint) {
                mGapToExpandHint = a.getString(attr);
            } else if (attr == R.styleable.TSExpandableTextView_tsGapToShrinkHint) {
                mGapToShrinkHint = a.getString(attr);
            } else if (attr == R.styleable.TSExpandableTextView_tsIsNeedShrink) {
    ...
    private void init() {
        ...
        if (TextUtils.isEmpty(mToExpandHint)) {
            mToExpandHint = getContext().getString(R.string.tsui_common_open);
        }
        if (TextUtils.isEmpty(mToShrinkHint) && mIsNeedShrink) {
            mToShrinkHint = getContext().getString(R.string.tsui_common_close);
        }
        if (mToggleEnable) {
    ...
    /**
     * used in ListView or RecyclerView to update TSExpandableTextView
     *
     * @param text                original text
     * @param futureTextViewWidth the width of TSExpandableTextView in px unit,
    ...
    /**
     * get the current state of TSExpandableTextView
     *
    ...
    public interface OnExpandListener {
        void onExpand(TSExpandableTextView view);

        void onShrink(TSExpandableTextView view);
    }
    ...
    public class TouchableSpan extends ClickableSpan {
        ...
        public void onClick(View widget) {
            if (hasOnClickListeners()
                    && (getOnClickListener(TSExpandableTextView.this) instanceof ExpandableClickListener)) {
            } else {
                toggle();
            }
        }
